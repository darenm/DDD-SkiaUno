...
namespace UnoSkia1
{
    public class PieChartView
    {
        ...
        public void Draw(SKCanvas canvas, SKPoint origin, float radius)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"animationFactor: {animationFactor}");

            var totalValue = Slices.Sum(x => x.Value);
            var translateThreshhold = 0.2f;
            float explodeOffset = animationFactor < 1f - translateThreshhold ? 0 : 50 * (1 - (1f - animationFactor) / translateThreshhold);
